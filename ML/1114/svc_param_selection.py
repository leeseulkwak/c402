...
df = pd.read_csv(os.path.join(cwd, "1114", "basketball_stat.csv"))
...
def svc_param_selection(X, y, nfolds):
    ...
    # C: SVM의 규제 파라미터로 6가지 값에 대해 탐색. 작은 값은 더 부드러운 결정 경계를, 큰 값은 더 복잡한 결정 경계를 만든다.
    svm_parameters = [
                        {'kernel': ['rbf'],
...
# 모델 저장
joblib.dump(clf, os.path.join(cwd, "1114", "svm_model.pkl"))

# 저장 확인
print("모델이 'svm_model.pkl' 파일로 저장되었습니다.")